...
const Sidebar = () => {
    ...
    return (
        <div className={`absolute z-[999] top-0 rounded-l-xl duration-700 bg-white md:w-1/3 w-10/12 h-fit border border-slate-200 ${isOpen === true ? 'left-0' : '-left-[900px] md:-left-[1500px] lg:-left-[2300px] xl:-left-[2700px]'}`}>
            ...
            <div className={`fixed top-0 right-0 left-0 ${isOpen ? "opacity-100" : "opacity-0 hidden"} duration-700 min-h-screen min-w-90% z-0 bg-black opacity-50`} onClick={() => setIsOpen(false)}></div>
        </div>
    )
}
