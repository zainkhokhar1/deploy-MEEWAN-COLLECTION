...
const PreviewItem = () => {
    ...
    const handleLike = (product) => {
        ...
        if (alreadyInWishlist) {
            wishList.filter((single) => single !== product.id);
            dispatch({
                type: 'delete',
                payload: product
            });
    ...
    useEffect(() => {
        if (wishList.length > 0 && product.length > 0) {
            const checking = wishList.some(singleItem => singleItem.id == product[0].id);
            setLike(checking);
        }
    }, [wishList, product]);
    ...
    useEffect(() => {
        if (product.length > 0 && localStorage.getItem('recentlyViewed')) {
            ...
            let id = product[0].id.toString();
            let Checking = oldData.some((one) => one === id);
            if (!Checking) {
                let newRecents = [...oldData, product[0].id.toString()];
            ...
            if (product && product.length > 0) {
                let newRecents = [product[0].id.toString()]
                localStorage.setItem('recentlyViewed', JSON.stringify(newRecents));
                setRecentlyViewed(newRecents);
            }
        }
    ...
    const addToCart = (product) => {
        ...
        let condition = cart.some((singleItem) => {
            return addedProduct.id === singleItem.id
        });
        if (condition) {
            let item = cart.filter((singleEntity) => addedProduct.id === singleEntity.id);
                ...
                <div className='flex flex-col'>
                    <div className='flex flex-col md:grid md:grid-cols-8'>
                        <div className='w-full h-96 lg:min-h-[38rem] xl:min-h-[32rem] md:min-h-[30rem] relative md:col-span-4 lg:col-span-3'>
                            <img className='w-full h-full object-cover pr-1 sm:pr-0' src={product[0].image} alt="ProductImage" />
                        ...
                        <div className='py-4 md:px-8 md:col-span-3 xl:pl-12 md:space-y-2'>
                            ...
                            <div className=''>
                                <span className='font-bold block'>SIZE : MEDIUM</span>
                                <button onClick={() => setSize(1)} className={`border ${size === 1 ? "bg-slate-800 text-white" : "border-red-600"}  rounded-full px-[6px] py-[2px] text-slate-500 mr-2 my-3`}>Medium</button>
