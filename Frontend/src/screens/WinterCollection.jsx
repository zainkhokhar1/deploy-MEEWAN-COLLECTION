...
const WinterCollection = () => {
  ...
  // Getting all the products based on the sortBy
  useEffect(() => {
    if (sortBy === "Alphabetically, A-Z") {
      let newProducts = [...winterCollection].sort((a, b) => {
        return b.title.toLowerCase() < a.title.toLowerCase() ? 1 : -1
      });
      setWinterCollection(newProducts);
    }
  ...
  // Getting all the products based on the filter
  useEffect(() => {
  ...
  return (
    <>
      {
        winterCollection.length > 0 ? <div className=''>
          <div className='bg-[#757575] h-9 w-full text-xl text-white flex items-center justify-center mb-9'>
            Winter Collection
          </div>
          <div className='flex justify-between xl:px-20 gap-9 xsm:gap-20 sm:gap-0 items-center px-2 w-full'>
            ...
            option === 1 ? <div className='grid grid-cols-2 px-2 my-20 xsm:gap-1 md:gap-4 xl:mx-20 lg:mx-5 lg:gap-7'>
              {
                winterCollection.length > 0 ?
                  winterCollection.map((winter) => {
                    return <div key={winter._id} className='flex items-center justify-center'> <SingleProduct type={1} wish={false} className="col-span-1" singleProduct={winter} />
                    </div>
                  })
                  : ""
              }
              ...
              {
                winterCollection.length > 0 ? winterCollection.map((winter) => <SecondSingleProduct wish={false} key={winter._id} singleProduct={winter} />) : ""
              }
            </div> : option === 3 ? <div className='grid grid-cols-1 px-3 my-20 mr-4 lg:mx-5 xl:px-20'>
              {
                winterCollection.length > 0 ? winterCollection.map((winter) => <ThirdSingleProduct wish={false} key={winter._id} singleProduct={winter} />) : ""
              }
            </div> : option === 4 ? <div className='grid grid-cols-3 px-3 my-20 gap-3 lg:mx-5 lg:gap-5 xl:px-20'>
              {
                winterCollection.length > 0 ? winterCollection.map((winter) => <SingleProduct type={2} wish={false} key={winter._id} singleProduct={winter} />) : ""

              }
            </div> : option === 5 ? <div className='grid grid-cols-4 px-3 my-20 gap-4 lg:mx-4 xl:px-20'>
              {
                winterCollection.length > 0 ? winterCollection.map((winter) => <SingleProduct type={3} wish={false} key={winter._id} singleProduct={winter} />) : ""
              }
            </div> : option === 6 ? <div className='grid grid-cols-5 px-3 my-20 gap-4 lg:mx-4 xl:px-20'>
              {
                winterCollection.length > 0 ? winterCollection.map((winter) => <SingleProduct type={4} wish={false} key={winter._id} singleProduct={winter} />) : ""
              }
            </div> : option === 7 ? <div className='grid grid-cols-6 px-3 my-20 gap-4 lg:mx-4 xl:px-20'>
              {
                winterCollection.length > 0 ? winterCollection.map((winter) => <SingleProduct type={5} wish={false} key={winter._id} singleProduct={winter} />) : ""
              }
            </div> : ""
          }
