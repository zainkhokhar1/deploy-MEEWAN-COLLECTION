...
const Women = () => {
  ...
  return (
    <div className=''>
      ...
      <div className='flex justify-between cursor-pointer xl:px-20 gap-9  xsm:gap-20 sm:gap-0 items-center px-2 w-full'>
        <div onClick={() => { setFilterOpen(!filterOpen); localStorage.setItem('filterSlider', true) }} className='flex items-center font-semibold md:justify-start justify-center pl-3 gap-1 text-slate-500 w-1/3'>
        ...
        option === 1 ? <div className='grid grid-cols-2 px-2 my-20 xsm:gap-1 md:gap-4 xl:mx-20 lg:mx-5 lg:gap-7'>
          {
            womens.length > 0 ?
              womens.map((women) => {
                return <div key={women.id} className='flex items-center justify-center'> <SingleProduct type={1} wish={false} className="col-span-1" singleProduct={women} />
                </div>
              })
              : ""
          }
          ...
          {
            womens.length > 0 ? womens.map((women) => <SecondSingleProduct wish={false} key={women.id} singleProduct={women} />) : ""
          }
        </div> : option === 3 ? <div className='grid grid-cols-1 px-3 my-20 mr-4 lg:mx-5 xl:px-20'>
          {
            womens.length > 0 ? womens.map((women) => <ThirdSingleProduct wish={false} key={women.id} singleProduct={women} />) : ""
          }
        </div> : option === 4 ? <div className='grid grid-cols-3 px-3 my-20 gap-3 lg:mx-5 lg:gap-5 xl:px-20'>
          {
            womens.length > 0 ? womens.map((women) => <SingleProduct type={2} wish={false} key={women.id} singleProduct={women} />) : ""

          }
        </div> : option === 5 ? <div className='grid grid-cols-4 px-3 my-20 gap-4 lg:mx-4 xl:px-20'>
          {
            womens.length > 0 ? womens.map((women) => <SingleProduct type={3} wish={false} key={women.id} singleProduct={women} />) : ""
          }
        </div> : option === 6 ? <div className='grid grid-cols-5 px-3 my-20 gap-4 lg:mx-4 xl:px-20'>
          {
            womens.length > 0 ? womens.map((women) => <SingleProduct type={4} wish={false} key={women.id} singleProduct={women} />) : ""
          }
        </div> : option === 7 ? <div className='grid grid-cols-6 px-3 my-20 gap-4 lg:mx-4 xl:px-20'>
          {
            womens.length > 0 ? womens.map((women) => <SingleProduct type={5} wish={false} key={women.id} singleProduct={women} />) : ""
          }
        </div> : ""
      }
