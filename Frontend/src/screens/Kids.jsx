        ...
        option === 1 ? <div className='grid grid-cols-2 px-2 my-20 xsm:gap-1 md:gap-4 xl:mx-20 lg:mx-5 lg:gap-7'>
          {
            kids.length > 0 ?
              kids.map((kid) => {
                return <div key={kid.id} className='flex items-center justify-center'> <SingleProduct type={1} wish={false} className="col-span-1" singleProduct={kid} />
                </div>
              })
              : ""
          }
          ...
          {
            kids.length > 0 ? kids.map((kid) => <SecondSingleProduct wish={false} key={kid.id} singleProduct={kid} />) : ""
          }
        </div> : option === 3 ? <div className='grid grid-cols-1 px-3 my-20 mr-4 lg:mx-5 xl:px-20'>
          {
            kids.length > 0 ? kids.map((kid) => <ThirdSingleProduct wish={false} key={kid.id} singleProduct={kid} />) : ""
          }
        </div> : option === 4 ? <div className='grid grid-cols-3 px-3 my-20 gap-3 lg:mx-5 lg:gap-5 xl:px-20'>
          {
            kids.length > 0 ? kids.map((kid) => <SingleProduct type={2} wish={false} key={kid.id} singleProduct={kid} />) : ""

          }
        </div> : option === 5 ? <div className='grid grid-cols-4 px-3 my-20 gap-4 lg:mx-4 xl:px-20'>
          {
            kids.length > 0 ? kids.map((kid) => <SingleProduct type={3} wish={false} key={kid.id} singleProduct={kid} />) : ""
          }
        </div> : option === 6 ? <div className='grid grid-cols-5 px-3 my-20 gap-4 lg:mx-4 xl:px-20'>
          {
            kids.length > 0 ? kids.map((kid) => <SingleProduct type={4} wish={false} key={kid.id} singleProduct={kid} />) : ""
          }
        </div> : option === 7 ? <div className='grid grid-cols-6 px-3 my-20 gap-4 lg:mx-4 xl:px-20'>
          {
            kids.length > 0 ? kids.map((kid) => <SingleProduct type={5} wish={false} key={kid.id} singleProduct={kid} />) : ""
          }
        </div> : ""
      }
